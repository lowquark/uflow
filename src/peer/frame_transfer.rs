...
impl ResendEntry {
    fn new(frame_data: Box<[u8]>, sequence_id: u32) -> Self {
        Self {
...
impl Tx {
    ...
    pub fn handle_data_ack(&mut self, data_ack: frame::DataAck) {
        ...
        for sequence_id in data_ack.sequence_ids.into_iter() {
            for (idx, entry) in self.resend_queue.iter().enumerate() {
    ...
    fn pull_frame(&mut self, now: time::Instant, timeout: time::Duration) -> Option<Box<[u8]>> {
        ...
        if self.next_sequence_id.wrapping_sub(self.base_sequence_id) < TRANSFER_WINDOW_SIZE {
            ...
            // Assemble a new data frame from as many send entries as possible
            while let Some(entry) = self.send_queue.front() {
                ...
                if num_entries == 0 || frame_size + encoded_size <= MTU {
                    ...
                    if reliable_entries.len() == 0 {
                        ...
                        self.resend_queue.push_back(ResendEntry::new(resend_frame.to_bytes(), sequence_id));
